...
function TarantoolConnection (options){
    ...
    this.namespace = {};
    this.bufferSlide = createBuffer(1024*1000);
    this.bufferOffset = 0;
    this.bufferLength = 0;
...
TarantoolConnection.prototype._addToInnerBuffer = function(data, from, size){
    ...
    console.log('data len', data.length, this.bufferLength);
    if (from && size)
    {
        if (this.bufferOffset + this.bufferLength + data.length < size)
        {
        ...
        else
        {
            ...
            if (this.bufferSlide.length > destLen)
            {
                ...
                {
                    if (this.bufferOffset > destLen)
                    {
                        this.bufferSlide.copy(this.bufferSlide, 0, this.bufferOffset, this.bufferLength);
                        data.copy(this.bufferSlide, this.bufferLength, from, from+size);
                    }
                    else
                    {
        ...
        else
        {
            ...
            if (this.bufferSlide.length > destLen)
            {
                if (this.bufferLength)
                {
                    ...
                    {
                        this.bufferSlide.copy(this.bufferSlide, 0, this.bufferOffset, this.bufferLength);
                        data.copy(this.bufferSlide, this.bufferLength);
                    }
                    else
                    {
                        var newBuffer = createBuffer(this.bufferSlide.length * multiplierBuffer);
    ...
    switch(this.state){
        ...
        case states.AWAITING:
            ...
            while(this.awaitingResponseLength >0 && this.awaitingResponseLength <= this.bufferLength)
            {
                ...
                this.bufferOffset = this.bufferOffset+this.awaitingResponseLength;
...
TarantoolConnection.prototype._processResponse = function(buffer, offset, length){
    ...
    console.log(this.n=(this.n && ++this.n)||1);
        decoder.buffer = buffer;
