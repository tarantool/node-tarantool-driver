...
var bufferSlide = Buffer.alloc(1024*10);
...
TarantoolConnection.prototype._processResponse = function(buffer, offset, length){
    if (offset)
    {
      ...
      dataBuffer[0]= 0x92;
      buffer.copy(dataBuffer, 1, offset, length);
    }
    else
    {
...
TarantoolConnection.prototype.select = function(spaceId, indexId, limit, offset, iterator, key){
    ...
    return new Promise(function(resolve, reject){
        ...
        if (iterator == 'all')
            key = [];
        var body = Buffer.concat([new Buffer([0x86,tarantoolConstants.KeysCode.space_id]), conn.msgpack.encode(spaceId),
