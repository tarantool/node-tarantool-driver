...
// Commands.prototype._header = function(command, reqId){
...
// Commands.prototype._request = function(header, body){
...
Commands.prototype.selectCb = function(spaceId, indexId, limit, offset, iterator, key, success, error){
	...
	//console.log(buffer);
	// this.commandsQueue.push([tarantoolConstants.RequestCode.rqSelect, reqId, {resolve: success, reject: error}]);
};
...
Commands.prototype.auth = function(username, password){
	return new Promise(function (resolve, reject) {
		...
		buffer[5] = 0x82;
		buffer[6] = tarantoolConstants.KeysCode.code;
		buffer[7] = tarantoolConstants.KeysCode.rqAuth;
		buffer[8] = tarantoolConstants.KeysCode.sync;
		buffer[9] = 0xce;
		buffer.writeUInt32BE(reqId, 10);
		buffer[14] = 0x82;
		...
		this.sendCommand([tarantoolConstants.RequestCode.rqAuth, reqId, {resolve: resolve, reject: reject}], buffer);
