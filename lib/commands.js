...
Commands.prototype._getSpaceId = function(name){
	if (!requestedSpaceIds[name] || !(requestedSpaceIds[name] instanceof Promise)) {
		_this = this;
		requestedSpaceIds[name] = this.select(tarantoolConstants.Space.space, tarantoolConstants.IndexSpace.name, 1, 0,
			'eq', [name])
			.then(function(value){
	...
	requestedSpaceIds[name].catch(() => { delete requestedSpaceIds[name] });
	return requestedSpaceIds[name];
};

Commands.prototype._getIndexId = function(spaceId, indexName){
	const name = `${spaceId}_${indexName}`;
	if (!requestedIndexIds[name] || !(requestedIndexIds[name] instanceof Promise)) {
		_this = this;
		requestedIndexIds[name] = this.select(tarantoolConstants.Space.index, tarantoolConstants.IndexSpace.indexName, 1, 0,
	...
	requestedIndexIds[name].catch(() => { delete requestedSpaceIds[name] });
	return requestedIndexIds[name];
};
